...
func TestDisableReshardOnRetry(t *testing.T) {
	...
	var (
		...
		client = &MockWriteClient{
			StoreFunc: func(ctx context.Context, b []byte, i int) (ResponseStats, error) {
				onStoreCalled()

				return ResponseStats{}, RecoverableError{
...
func (c *TestWriteClient) Store(_ context.Context, req []byte, _ int) (ResponseStats, error) {
	c.mtx.Lock()
	defer c.mtx.Unlock()
	if c.storeWait > 0 {
	...
	if c.returnError != nil {
		return ResponseStats{}, c.returnError
	}
	...
	for _, ts := range reqProto.Timeseries {
		...
		rs.AcceptedHistograms += len(ts.Histograms)
	}
...
func (c *TestBlockingWriteClient) Store(ctx context.Context, _ []byte, _ int) (ResponseStats, error) {
	c.numCalls.Inc()
	<-ctx.Done()
...
func (c *NopWriteClient) Store(context.Context, []byte, int) (ResponseStats, error) {
	return ResponseStats{}, nil
}
...
type MockWriteClient struct {
	StoreFunc    func(context.Context, []byte, int) (ResponseStats, error)
	NameFunc     func() string
	EndpointFunc func() string
}

func (c *MockWriteClient) Store(ctx context.Context, bb []byte, n int) (ResponseStats, error) {
